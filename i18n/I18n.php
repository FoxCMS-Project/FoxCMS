<?php
/*
 |  A I18N helper class (Deprecated, please use Gettext instead)!
 |  @file       ./I18n.php
 ...
 */
    ...
    class I18n{
        /*
         |  GLOBAL VARs
         */
        static private $path = NULL;            // The basic path to the I18N files
        static private $default = NULL;         // The default language / locale

        static private $locale = NULL;          // The current language / locale
        static private $strings = array();      // The translation strings
        static private $defaults = array();     // The default strings
        ...
        /*
         ...
         |  @since  0.2.0
         |
         |  @param  multi   The default locale path to fetch the string files.
         |  @param  multi   The default locale code.
         |  @param  multi   The current locale code.
         |
         |  @return bool    TRUE on success, FALSE on failure.
         */
        static public function init($path, $default = NULL, $locale = NULL){
            if(file_exists($path) && is_dir($path)){
                self::$path = realpath($path) . DIRECTORY_SEPARATOR;
            }
            if(self::isLanguage($default)){
                self::$default = $default;
            }
            return self::setLocale(self::isLocale($locale)? $locale: $default) > 0;
        }
        ...
        /*
         ...
         |  @update 0.2.0
         |
         |  @param  multi   The respective locale to set or NULL to use the default one.
         |
         |  @return bool    TRUE on sucess, FALSE on failure.
         */
        static public function setLocale($locale = NULL){
            self::$locale = empty($locale)? self::$default: $locale;

            // Load Default Strings
            $defaults = self::$path . self::$default;
            while(true){
                ...
                if(file_exists($defaults . "-message.php")){
                    $defaults = include($defaults . "-message.php");
                    break;
                }

                // Load JSON File
                if(file_exists($defaults . ".json")){
                    $defaults = $defaults . ".json";
                } else if(file_exists($defaults . "-message.json")){
                    $defaults = $defaults . "-message.json";
                } else {
                    break;
                }
                $defaults = file_get_contents($defaults . "-message.json");
            ...
            $strings = self::$path . self::$locale;
            while(true){
                if(!empty(self::$strings)){
                    self::$strings = array();
                }
                if(self::$default == self::$locale){
                    self::$strings = self::$defaults;
                    break;
                }

                // Load PHP File
                if(file_exists($strings . ".php")){
                    $strings = include($strings . ".php");
                    break;
                }
                if(file_exists($strings . ".php")){
                    $strings = include($strings . "-message.php");
                    break;
                }

                // Load JSON File
                if(file_exists($strings . ".json")){
                    $strings = $strings . ".json";
                } else if(file_exists($strings . "-message.json")){
                    $strings = $strings . "-message.json";
                } else {
                    break;
                }
                $strings = file_get_contents($strings . "-message.json");
            ...
            if(is_array($defaults)){
                return self::add($strings, true) > 0;
            }
            return false;
        }
        ...
        /*
         ...
         |  @update 0.2.0
         |
         |  @param  bool    TRUE to return the default locale, FALSE to return the current.
         |
         |  @return string  The current locale string.
         */
        static public function getLocale($default = false){
            return ($default)? self::$default: self::$locale;
        }
        ...
        /*
         |  CHECK IF LOCALE EXISTS / IS AVAILABLE
         |  @since  0.2.0
         |
         ...
         |  @return bool    TRUE on success, FALSE on failure.
         */
        static public function isLocale($locale, $available = false){
        ...
        static public function isLanguage($locale, $available = false){
            return self::isLocale($locale, $available);
        }
